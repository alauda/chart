...
export function axesSpace(self: uPlot, axisIdx: number, scaleMin: number) {
  ...
  if (scale.time) {
    return measureText(String(scaleMin), UPLOT_AXIS_FONT_SIZE).width + 18;
  }
 ...
 * @beta
 */
export function measureText(text: string, fontSize = 12): TextMetrics {
  const fontStyle = `${fontSize}px "Roboto", "Helvetica", "Arial", sans-serif`;
